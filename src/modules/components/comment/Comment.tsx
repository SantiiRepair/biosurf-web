...
function Comment(
  ...
  const renderCommentEditorByMode = (editorMode: any) => {
    if (editorMode === "edit" && !parentId)
      return (
        <CommentEditor
          id={id}
          isUpdate={editorMode === "edit"}
    ...
    return (
      window.screen.width > 960 && (
        <Box marginRight={5} width={"40px"}>
          <Flex
            className="score_counter_desktop"
            alignItems={"center"}
            direction="column"
          ...
          >
            <AddIcon
              ...
              cursor={"pointer"}
              onClick={() => incrementUpvote(id, parentId)}
              _hover={{ color: "moderate_blue" }}
            />
            <Text fontWeight={"bold"} color={"moderate_blue"} mx={4}>
              {score}
            </Text>
            <MinusIcon
              as={"button"}
              ...
              color={"light_grayish_blue"}
              cursor={"pointer"}
              onClick={() => decrementUpvote(id, parentId)}
              _hover={{ color: "moderate_blue" }}
            />
          </Flex>
        </Box>
          ...
          <Box width={"100%"}>
            <Flex alignItems={"center"} justifyContent="space-between" flex={1}>
              ...
              <Flex className="user_details" alignItems={"center"}>
                ...
                {/* Date Posted */}
                <Text fontWeight={"bold"} color={"grayish_blue"} mx={3}>
            ...
            <Text color={"grayish_blue"} fontWeight={"normal"} my={5}>
              {replyingTo && (
                <Text as={"span"} fontWeight={"bold"} color="moderate_blue">
                  @{replyingTo}{" "}
                </Text>
              )}
              {content}
